...
int StkWrap::OnGotLevel2Record(GTLevel2 *pRcd)
{
	...
	if (tl->_depth==0)
	{
        ...
        if (pRcd->chSide=='B')
        {
				...
				excp(k.be ,CAST_MMID_TEXT(pRcd->mmid));
        }
        else
        {
                k.ask = pRcd->dblPrice;
                k.os = pRcd->dwShares / SHAREUNITS;
				excp(k.oe ,CAST_MMID_TEXT(pRcd->mmid));
...
void StkWrap::SendData()
{
	...
	for(int i = SENDDEPTH-1; i >=0 ; i--)
	{
		...
		if( (wasChange && i == 0) || gotSome )
		{
			...
			excp(k.oe ,"");//CString(m_level1.locAskExchangeCode);
...
#endif
			excp(k.be ,"");//CString(m_level1.locBidExchangeCode);
			gotSome = true;
