...
namespace TradeLink.Research
{
    ...
    public class FetchTarget
    {
        ...
        public Basket FetchURL()
        {
            ...
            if (_xdupe) mb = BasketImpl.RemoveDupe(mb);
            return mb;
        }
        /// <summary>
        ...
        public Basket FetchFILE()
        {
            ...
            if (_nyse && _linkedonly) mb.Add(BasketImpl.LinkedOnlyNYSE(file));
            else if (_nyse) mb.Add(BasketImpl.NYSE(file));
            if (_nasd && _linkedonly) mb.Add(BasketImpl.LinkedOnlyNASDAQ(file));
            else if (_nasd) mb.Add(BasketImpl.NASDAQ(file));
            if (_xdupe) mb = BasketImpl.RemoveDupe(mb);
            return mb;
