...
namespace TradeLink.Common
{
    public class BrokerFeed : TLClient
    {
        ...
        void initsot()
        {
            if ((sot == null) && isExecuteSubscribing)
            {
                sot = new SendorderTracker(execute, 100);
                sot.SubscribeBeforeSend = isExecuteSubscribing;
                sot.SendDebugEvent+=new DebugDelegate(debug);
                sot.OrderIdStatusEvent += new OrderId_Status(sot_OrderIdStatusEvent);
                sot.ResendOnSelectError = true;
                sot.SelectedErrors.Add((int)MessageTypes.SYMBOL_NOT_LOADED);
        ...
        Basket _curbask = new BasketImpl();
        public Basket SubscribedBasket { get { return _curbask; } }
        /// <summary>
        ...
        public void Subscribe(Basket b)
        {
            if (quote == null) 
                return;
            _curbask.Add(b);
            v("subscribing via feed to: " + _curbask.ToString());
            quote.Subscribe(_curbask);
        }
        /// <summary>
        ...
        public int ExecuteSubscribePauseMs { get { return sot == null ? 0 : sot.SubscribePause; } set { if (sot == null) return; sot.SubscribePause = value; } }

        bool _isexecsub = false;
        public bool isExecuteSubscribing { get { return _isexecsub; } set { _isexecsub = value; initsot(); } }
        ...
        {
            gotquotefeatures = true;
            _RequestFeaturesList.AddRange(messages);
            notifyfeatures();
        }

        bool gotquotefeatures = false;
        bool gotexecfeatures = false;
        void notifyfeatures()
        {
            if (gotquotefeatures && gotexecfeatures && (gotFeatures != null))
                gotFeatures(_RequestFeaturesList.ToArray());
        }

        void execute_gotFeatures(MessageTypes[] messages)
        {
            gotexecfeatures = true;
            _RequestFeaturesList.AddRange(messages);
            notifyfeatures();
        }
        ...
        public event DebugDelegate gotServerUp { add { } remove { } }
        public event DebugDelegate gotServerDown { add { } remove { } }
