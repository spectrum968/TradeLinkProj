...
namespace TradeLink.Common
{
    ...
    /// </summary>
    [Serializable]
    public class OrderImpl : TradeImpl, Order, TradeLink.API.SymbolI
    {
        ...
        [Obsolete("You should use Order.ValidInstruct instead")]
        public string TIF { get { return _instruct.ToString().Replace("OrderInstructionType", string.Empty).Replace(".", string.Empty); } set { } }
        ...
        public OrderImpl(Order copythis)
        {
            ...
            this.id = copythis.id;
            this.ValidInstruct = copythis.ValidInstruct;
        }
        ...
        /// <returns></returns>
        public bool Fill(Tick k, bool bidask, bool fillOPG, bool fillHighLiquidityEOD)
        {
            if (!fillHighLiquidityEOD || (!isStop && !isLimit))
                return Fill(k, bidask, fillOPG);

            if (k.symbol != symbol) 
                return false;
            if (!fillOPG && (this.ValidInstruct== OrderInstructionType.OPG)) 
                return false;
            ...
            else if( (isStop && side && (p >= stopp)) // buy stop
                  || (isStop && !side && (p <= stopp))) // sell stop
            {
                this.xprice = stopp;
                return true;
        ...
        public new static Order Deserialize(string message)
        {
            ...
            OrderInstructionType oit = OrderInstructionType.DAY;
            if (Enum.TryParse<OrderInstructionType>(rec[(int)OrderField.OrderTIF],out oit))
                o.ValidInstruct = oit;
            decimal trail = 0;
