...
namespace TradeLink.Common
{
    ...
    {
        public BuyOPG(string sym, int size, decimal price, string comment) : base(sym,true,size,price,0,comment,0,0) { ValidInstruct = API.OrderInstructionType.OPG; }
        public BuyOPG(string sym, int size,decimal price) : this(sym,size,price,"") {}
        public BuyOPG(string sym, int size, decimal price, long orderid) : base(sym, true, size, price, 0, string.Empty, 0, 0, orderid) { ValidInstruct = API.OrderInstructionType.OPG; }

    }

    public class SellOPG : OrderImpl
    {
        public SellOPG(string sym, int size, decimal price, string comment) : base(sym, false, size, price, 0, comment, 0, 0) { ValidInstruct = API.OrderInstructionType.OPG; }
        public SellOPG(string sym, int size, decimal price) : this(sym, size, price, "") { }
        public SellOPG(string sym, int size, decimal price, long orderid) : base(sym, false, size, price, 0, string.Empty, 0, 0, orderid) { ValidInstruct = API.OrderInstructionType.OPG; }

    }

    public class OPGOrder : OrderImpl
    {
        public OPGOrder(string sym, bool side, int size, decimal price, string comment) : base(sym, side, size, price, 0, comment, 0, 0) { ValidInstruct = API.OrderInstructionType.OPG; }
        public OPGOrder(string sym, bool side, int size, decimal price) : this(sym, side, size, price, "") { }
        public OPGOrder(string sym, bool side, int size, decimal price, long orderid) : base(sym, side, size, price, 0, string.Empty, 0, 0, orderid) { ValidInstruct = API.OrderInstructionType.OPG; }
    }
