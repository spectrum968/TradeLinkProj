...
namespace TradeLink.Common
{
    ...
    public class OrderTracker  : GotOrderIndicator,GotCancelIndicator,GotFillIndicator,GenericTrackerI
    {
        ...
        {
            return orders.addindex(txt, new OrderImpl(txt, 0));
        }
        ...
        void orders_NewTxt(string txt, int idx)
        {
            ...
            orgid.addindex(txt, orders[idx].id);
        ...
        GenericTracker<long> orgid = new GenericTracker<long>();
        ...
        {
            // verify valid
            if ((idx < 0) || (idx >= orgid.Count))
            {
                debug("Can't get order id for invalid index: " + idx);
                return 0;
            }
            return orgid[idx];
        }
        ...
        bool _noverb = true;
        public virtual bool VerboseDebugging
        {
            get { return !_noverb; }
            set
            {
