...
namespace TradeLink.Common
{
    ...
    public class TLServer_WM : Form, TLServer
    {
        ...
        public event PositionAccountArrayDelegate newPosList;
        ...
        {
            MinorVer = Util.ProgramBuild(Util.PROGRAM);
        ...
        {
            TLSend_Handle(msg, type, self,hims[clientid]);
...
#if DEBUG
                if (VerboseDebugging)
                    debug(Text + " could not find client: " + clientname + " to send: " + type + " with: " + message);
#endif
                return;
            }
            var dest = hims[id];

            TLSend_Handle(message,type,self,dest);
        }
        ...
        delegate void tlsenddel(string m, MessageTypes t, IntPtr source, IntPtr dest);
        ...
        {
                for (int i = 0; i < client.Count; i++)
                    TLSend_Handle(ImbalanceImpl.Serialize(imb), MessageTypes.IMBALANCERESPONSE,self, hims[i]);
        }
