...
namespace TradeLink.Common
{
    ...
    public class HistSimIndex
    {
        ...
        /// <param name="hsi"></param>
        /// <returns></returns>
		public static bool BuildIndex(string folder, string[,] tickfiles, TickFileFilter tff, out HistSimIndex hsi, bool start, bool bUseCSV) { return BuildIndex(folder, tickfiles, out hsi, start, true, 0, null, bUseCSV); }
        public static bool BuildIndex(string folder, string[,] tickfiles, TickFileFilter tff, out HistSimIndex hsi, bool start,bool saveidx, int interval, DebugDelegate debug)
        {
            ...
            return BuildIndex(folder,allows, out hsi,start,saveidx,interval,debug,tff.bUseCSV);
        }
        /// <summary>
        ...
        public static bool BuildIndex(string folder, string[,] tickfiles, TickFileFilter tff, out HistSimIndex hsi, int interval, DebugDelegate debug)
        {
            ...
            return BuildIndex(folder,allows, out hsi, true,true,interval,debug,tff.bUseCSV);
        }
        /// <summary>
        ...
        /// <param name="hsi"></param>
        /// <returns></returns>
		public static bool BuildIndex(string folder, string[,] tickfiles, out HistSimIndex hsi, bool bUseCSV) { return BuildIndex(folder, tickfiles, out hsi, true, bUseCSV); }
        /// <summary>
        ...
        /// <returns></returns>
		public static bool BuildIndex(string folder, string[,] tickfiles, out HistSimIndex hsi, bool start, bool bUseCSV) { return BuildIndex(folder, tickfiles, out hsi, start, true, 0, null, bUseCSV); }
		public static bool BuildIndex(string folder, string[,] tickfiles, out HistSimIndex hsi, bool start, bool saveindex, bool bUseCSV) { return BuildIndex(folder, tickfiles, out hsi, start, saveindex, 0, null, bUseCSV); }
        public static bool BuildIndex(string folder, string[,] tickfiles, out HistSimIndex hsi, bool start, bool saveindex, int interval, DebugDelegate debug, bool bUseCSV)
        {
        ...
        /// build the index
        /// </summary>
        public void Start(bool bUseCSV)
        {
            ...
            for (int i = 0; i < TOC.GetLength(0); i++)
            {
                ...
                if (File.Exists(fn))
                    readers.Add(new tickreader(i, fn, _binaryindex, Interval, bUseCSV));
                else
                {
                    // try default path
            ...
            debug("Started "+readers.Count+" readers, waiting for completion");
        ...
        public static bool HaveIndex(string folder, TickFileFilter tff, out string file, DebugDelegate debug)
        {
            ...
            return HaveIndex(folder, filtered, out file, debug, tff.bUseCSV);
        ...
        /// <param name="debug"></param>
        /// <returns></returns>
        public static bool HaveIndex(string folder, string[,] tickfiles, out string file, DebugDelegate debug, bool bUseCSV)
        {
            return HaveIndex(folder, tickfiles, out file, 0, debug, bUseCSV);
        }
        public static bool HaveIndex(string folder, string[,] tickfiles, out string file,int interval,DebugDelegate debug, bool bUseCSV)
        {
            HistSimIndex hsi;
            if (BuildIndex(folder,tickfiles,out hsi,false,false,interval,debug,bUseCSV))
            {
        ...
        /// <param name="file"></param>
        /// <returns></returns>
        public static bool HaveIndex(string folder,string[,] tickindex, int interval,out string file,bool bUseCSV)
        {
            HistSimIndex hsi;
            if (BuildIndex(folder,tickindex,out hsi, false,false,interval,null,bUseCSV))
            {
                string checksum = MD5(indextoc(hsi));
                string fn = idxfold(folder) + checksum + ".txt";
                if (File.Exists(fn))
        ...
        internal class tickreader 
        {
            ...
            internal tickreader(int Index, string filepath, bool saveticks, int interval, bool bUseCSV)
            {
				b_UseCSV = bUseCSV;
                _int = interval;
                file = filepath;
				...
				if (bUseCSV)
				{
					InitCSVReader(Index, filepath, saveticks, interval);
				}
				else
				{
					InitTIKReader(Index, filepath, saveticks, interval);
				}                
