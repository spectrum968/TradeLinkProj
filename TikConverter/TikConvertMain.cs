...
namespace TikConverter
{
    ...
    public partial class TikConvertMain : AppTracker
    {
        ...
        public TikConvertMain()
        {
            ...
            _con.Text = Converter.GenericCSV.ToString();
            bw.WorkerReportsProgress = true;
            bw.DoWork += new DoWorkEventHandler(bw_DoWork);
            bw.RunWorkerCompleted += new RunWorkerCompletedEventHandler(bw_RunWorkerCompleted);
        }

        ConvertMap genericcsv = null;
        bool isgenericbusy = false;
        void dogenericconvert()
        {
            if (genericcsv == null)
            {
                debug("Generic CSV converter was not initialized.  Contact http://community.tradelink.org");
                return;
            }
            isgenericbusy = true;
            genericcsv.ConvertFiles(progress);
            isgenericbusy = false;
        }
        ...
        {
            // redirect for CSV - we want a function that parses OHLC and evenly divides the
            // volume amongst the ticks, rather than dropping all but the close
            if (con == Converter.CSV_DTOHLCV)
                return CSV_DTOHLCV_convert(filename, tradesize, sym);
            ...
            try
            {
                ...
                switch (con)
                {
                    case Converter.ThompsonReuters:
                    ...
                    case Converter.MultiCharts:
                        ...
                        break;
                    case Converter.TrueFX:
                        // This takes zip-files downloaded from TrueFX.com as input
                        // The symbol for data being imported is obtained from the filename
                        // Do not rename files downloaded from TrueFX.com
                        ...
                        using (Ionic.Zip.ZipFile zipfile = Ionic.Zip.ZipFile.Read(filename))
                        {
                            ...
                            zipentry.Extract(path, Ionic.Zip.ExtractExistingFileAction.OverwriteSilently);
                            filename = path + Path.DirectorySeparatorChar + zipentry.FileName;
                        }
                        infile = new StreamReader(filename);
                        break;
                 }
            ...
            { 
                debug("error reading input header:" + ex.Message); 
                g = false; 
            }
                ...
                try
                {
                    ...
                    switch (con)
                    {
                        case Converter.ThompsonReuters:
                            ...
                            break;
                        case Converter.TrueFX:
                            k = TrueFX.parseline(infile.ReadLine(), sym);
                            break;
                        case Converter.CQG:
        ...
        void progress(double percent)
        {
            ...
            if (statusStrip1.InvokeRequired)
                statusStrip1.Invoke(new DoubleDel(progress), new object[] { percent });
            else
            {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                System.Diagnostics.Process.Start(Util.TLTickDir);
            }
            catch
            {
                debug("An error occured locating your tick data folder.");
                debug("Try going to start -> programs -> tradelink -> tick data");
            }
        }
