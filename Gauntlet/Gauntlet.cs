...
namespace WinGauntlet
{
    public partial class Gauntlet : AppTracker
    {
        ...
        private void queuebut_Click(object sender, EventArgs e)
        {
			...
			// temp hack, create a response

			args.Response = new RBResponse();
			bindresponseevents();
		...
		private void _useCsvTicks_CheckedChanged(object sender, EventArgs e)
		{
			tickFileFilterControl1.bUseCSV = _useCsvTicks.Checked;
			if (_useCsvTicks.Checked && args.Folder.Length > 3 )
			{
				tickFileFilterControl1.SetSymbols(args.Folder);
			}
		}
