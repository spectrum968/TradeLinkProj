...
namespace TestTradeLink
{
    ...
    public class TestTikWriterReader
    {
        ...
        public void Appending()
        {
            ...
            {
                data[i].date = DATE;
                data[i].time = Util.ToTLTime(start);
            ...
            // verify length
            Assert.AreEqual(firsttestlen + data.Length,readdata.Count,"post-append data length not as expected");
        ...
        double writeandread(Tick[] data, int date, bool printperf) { return writeandread(data, date, printperf, true); }
